...
public class DeliverRepairRequestStatusImpl extends ServiceBase
{
    ...
    {
        getConnection();
        String sql = (new StringBuilder()).append("update SERVICE_INFO_TABLE SET finalPayment = ?, serviceStatus = ? , isOutwardCourier =?, outwardCourierDocumentNo = ?,  outwardCourierName = ?, outwardCourierPhone = ? , service_completion_date = ? where service_order_number = '").append(finalServiceNumber).append("' ").toString();
